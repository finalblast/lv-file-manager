<template>
  ...
  <div class="fm-grid">
    <div class="d-flex align-content-start flex-wrap" v-if="!isLoading">
      ...
      <div class="fm-grid-item text-center unselectable" v-for="(file, index) in files" v-bind:key="`f-${index}`" v-bind:title="file.basename" v-bind:class="{'active': checkSelect('files', file.path)}" v-on:click="selectItem('files', file.path, $event)" v-on:dblclick="emitInsertEvent()" v-on:contextmenu.prevent="contextMenu(file, $event)">
        <div class="fm-item-icon">
          <template v-if="thisImage(file.extension)">
            <img class="img-thumbnail" v-bind:alt="file.filename" v-bind:src="createImgUrl(file.path)">
          </template>
          <template v-else>
...
export default {
  ...
  methods: {
     ...
     */
    createImgUrl(path) {
      return `${location.protocol}//${location.hostname}/file-manager/thumbnails?disk=${this.disk || 'images'}&path=${path}`;
    },
